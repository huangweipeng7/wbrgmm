...
%  Precomputed normalization constant Z
%% Output:
...
%% Initialize:
    ...
    for iter = 2:(B + nmc) 
        ...
        for i = 1:n
            ...
            % Label the unique values of (theta_1,...,theta_n) as theta_star
            N = zeros(1, t);
            for k = 1:t
                j = 1;
    ...
    %             end
        ...
        for K = t:(t + m - 1)
            log_prob(K - t + 1) = Zhat(K - t + 1) - Z(K) + gammaln(K + 1)...
            - gammaln(K - t + 1) - gammaln(K + n + 1);
        end
        log_prob = log_prob - max(log_prob); 
        pr = exp(log_prob);
        pr = pr/sum(pr);
        ...
        % Given Gamma_k_star's, sample gamma_k_star's using accept-reject sampler
        g = zeros(K, K);
        count = 0;
        while (rand(1) >= min(min(g)))
            count = count + 1;
        ...
        K_mc(iter) = K;
        if (mod(iter, 50) == 0) 
            fprintf('Iteration #%d; Time Consumed: %f\n', iter, toc);
        end
    end
